...
async def test_server_broadcasts_event_after_handshake():
    ...
    try:
        async with connect(f"ws://{HOST}:{server.bound_port}") as ws:
            await ws.send(json.dumps({"id":"h","type":"hello","payload":{"protocol":1}}))
            assert json.loads(await ws.recv())["ok"] is True
            await server.broadcast("library.changed", {"media_type":"game"})
            event = json.loads(await ws.recv())
            assert event == {"type":"library.changed","payload":{"media_type":"game"}}
    finally:
        await server.close()
