...
def test_saved_godot_path_has_highest_priority(tmp_path):
    saved = tmp_path / "Godot_v4.7.2-stable_win64.exe"
    env = tmp_path / "env.exe"
    saved.write_bytes(b"")
    env.write_bytes(b"")
    settings = TerminalSettings.default()
    settings.godot_executable = str(saved)

    result = resolve_godot_executable(
        settings,
        environment={"GODOT_EXE": str(env)},
        which=lambda _name: None,
        chooser=lambda: None,
    )
    assert result == saved.resolve()
...
def test_environment_path_is_used_and_persisted(tmp_path):
    ...
    result = resolve_godot_executable(
        settings,
        environment={"GODOT_EXE": str(exe)},
        which=lambda _name: None,
        chooser=lambda: None,
    )
    assert result == exe.resolve()
    assert settings.godot_executable == str(exe.resolve())
...
def test_path_lookup_is_used_before_chooser(tmp_path):
    ...
    result = resolve_godot_executable(
        settings,
        environment={},
        which=lambda name: str(exe) if name == "godot" else None,
        chooser=chooser,
    )
    assert result == exe.resolve()
...
def test_missing_godot_raises_clear_error():
    settings = TerminalSettings.default()
    with pytest.raises(GodotDiscoveryError):
...
def test_build_godot_command_runs_project_directly(tmp_path):
    exe = tmp_path / "Godot.exe"
    project = tmp_path / "g3_frontend"
    exe.write_bytes(b"")
    project.mkdir()
    command = build_godot_command(exe, project)
    assert command == [str(exe.resolve()), "--path", str(project.resolve())]
    assert "--editor" not in command
