...
def test_carousel_does_not_consume_unrelated_left_clicks():
    ...
    left_block = re.search(r'if mouse_event\.pressed and mouse_event\.button_index == MOUSE_BUTTON_LEFT:(.*?)(?=\n    if event is InputEventKey:)', text, flags=re.S)
    assert left_block is not None
    body = left_block.group(1)
    assert "if _handle_left_click(mouse_event.position):" in body
    assert re.search(r'if _handle_left_click\(mouse_event\.position\):\s*\n\s*get_viewport\(\)\.set_input_as_handled\(\)', body)

def test_carousel_skips_mouse_input_when_hidden_or_empty():
    assert 'if not visible or games.is_empty():' in SOURCE.read_text(encoding="utf-8")
