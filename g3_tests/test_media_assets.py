...
def test_manual_preview_video_overrides_auto_discovery(tmp_path):
    repo = _repo(tmp_path)
    exe = tmp_path / "demo.exe"; exe.write_bytes(b"")
    auto_video = tmp_path / "preview.ogv"; manual_video = tmp_path / "chosen.ogv"
    auto_video.write_bytes(b"a"); manual_video.write_bytes(b"m")
...
def test_mp4_requests_ogv_transcode_command(tmp_path):
    repo = _repo(tmp_path)
    exe = tmp_path / "demo.exe"; exe.write_bytes(b"")
    video = tmp_path / "preview.mp4"; video.write_bytes(b"fake")
    game = repo.create_game(CreateGame(title="Demo", executable_path=exe))
    repo.add_media_asset(game.id, "preview_video", video, source="manual")
    calls = []
    def runner(command, **kwargs):
        calls.append(command); Path(command[-1]).write_bytes(b"ogv")
        return subprocess.CompletedProcess(command, 0, "", "")
    service = MediaAssetService(repo, tmp_path / "cache", ffmpeg_path="ffmpeg", runner=runner)
    manifest = service.resolve_preview(game.id)
    assert manifest.video_ogv is not None and manifest.video_ogv.suffix == ".ogv"
    assert calls
    command = calls[0]
    assert "libtheora" in command and "libvorbis" in command
