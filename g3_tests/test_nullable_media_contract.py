...
def _paths(root: Path) -> TerminalPaths:
    return TerminalPaths(root=root,database=root/"library.db",assets=root/"assets",cache=root/"cache",themes=root/"themes",logs=root/"logs",settings=root/"settings.json")


def test_game_payload_uses_empty_string_for_missing_cover(tmp_path):
    app = BackendApplication(_paths(tmp_path), builtin_theme_root=tmp_path/"builtin"); app.initialize()
    game = app.repository.create_game(CreateGame(title="RainWorld", executable_path=Path(sys.executable)))
    assert app._game_to_dict(game)["cover"] == ""
...
def test_preview_payload_uses_empty_strings_for_missing_optional_paths():
    ...
    assert payload["cover"] == "" and payload["background"] == "" and payload["video_ogv"] == "" and payload["preview_audio"] == "" and payload["logo"] == ""


def test_game_case_guards_null_cover_before_string_conversion():
    text=(Path(__file__).parents[1]/"g3_frontend"/"scripts"/"game_case_3d.gd").read_text(encoding="utf-8")
