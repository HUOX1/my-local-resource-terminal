...
class GigaCoverDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        safety = QLabel("安全规则：原图永不修改；源目录和输出目录不能相同；默认遇到已有正式封面会跳过。")
    ...
    def _scan(self) -> None:
        ...
        try:
            self._results = self.cropper.scan_directory(
                source,
                output,
        ...
        self.summary_label.setText(
            f"共 {len(self._results)} 张 · 可自动处理 {ready} · 需复核 {review} · 跳过 {skipped} · 错误 {errors}"
        )
