...
class SidebarToggleHandle(QSplitterHandle):
    """Transparent seam with a directional trapezoid toggle painted into one side."""
    ...
    def _trapezoid_points(self) -> QPolygonF:
        width = max(1.0, float(self.width()))
        height = max(1.0, float(self.height()))
        shape_w = min(float(self.SHAPE_WIDTH), max(12.0, width - 2.0))
        shape_h = min(float(self.SHAPE_HEIGHT), max(28.0, height - 8.0))
        top = (height - shape_h) / 2.0
        bottom = top + shape_h
        taper = min(6.0, shape_h * 0.16)

        if self._expanded:
            # Open rail: the trapezoid visually lives on the rail/left side and
            # tapers toward the rail, matching the left-pointing arrow.
            left = 0.5
            right = min(width - 0.5, left + shape_w)
            return QPolygonF(
                [
                    QPointF(left, top + taper),
                    QPointF(right, top),
                    QPointF(right, bottom),
                    QPointF(left, bottom - taper),
                ]
            )

        # Compact rail: move the same control to the content/right side and
        # taper toward the content, matching the right-pointing arrow.
        right = width - 0.5
        left = max(0.5, right - shape_w)
        return QPolygonF(
            [
                QPointF(left, top),
                QPointF(right, top + taper),
                QPointF(right, bottom - taper),
                QPointF(left, bottom),
            ]
        )
    ...
    def paintEvent(self, event) -> None:  # noqa: N802 - Qt override
        ...
        # surfaces. This makes the trapezoid read as physically sitting in the
        # rail when expanded and in the content area when compact, without a
        ...
        polygon = self._trapezoid_points()
        ...
        painter.drawText(bounds, Qt.AlignmentFlag.AlignCenter, arrow)
        painter.end()
    ...
    # The seam itself is not draggable. The whole trapezoid is a two-state
    # control, so the rail can never be left at an intermediate width.
    def mousePressEvent(self, event) -> None:  # noqa: N802 - Qt override
        if (
            event.button() == Qt.MouseButton.LeftButton
            and self._trapezoid_points().containsPoint(event.position(), Qt.FillRule.WindingFill)
        ):
            self._pressed = True
            self.update()
        event.accept()
