...
class DiscoveryService:
    ...
    def _walk(self, folder: Path, results: list[MovieCandidate]) -> None:
        try:
            ...
            return
        videos = [p for p in children if p.is_file() and p.suffix.casefold() in VIDEO_EXTENSIONS]
        if videos:
            main = self._select_main_video(folder, videos)
            subtitles = self._find_subtitles(children, main)
            results.append(
                MovieCandidate(
                    folder=folder,
                    video_path=main,
                    cover_key=main.stem,
                    inferred_code=main.stem,
