...
def test_v0419_is_visible_in_project_window_and_chrome():
    assert 'version = "0.4.3.1.1"' in (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert 'v0.4.3.1.1' in (ROOT / "app" / "ui" / "main_window.py").read_text(encoding="utf-8")
    assert 'v0.4.3.1.1' in (ROOT / "app" / "ui" / "app_chrome.py").read_text(encoding="utf-8")
