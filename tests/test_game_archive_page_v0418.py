...
def test_v0418_version_is_visible_in_project_and_window_chrome() -> None:
    assert 'version = "0.4.3.1.1"' in Path("pyproject.toml").read_text(encoding="utf-8")
    assert "v0.4.3.1.1" in Path("app/ui/main_window.py").read_text(encoding="utf-8")
    assert "v0.4.3.1.1" in Path("app/ui/app_chrome.py").read_text(encoding="utf-8")
