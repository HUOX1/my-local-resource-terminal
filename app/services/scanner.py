...
class Scanner:
    ...
    def _match_or_create(
        ...
        # Prefer the work folder name for segmented releases (for example
        # SPSA-01/SPSA-01_01.mp4). The folder represents the work, while the
        # file name often represents only a part.
        folder_key = candidate.video_path.parent.name.strip()
        if folder_key:
            by_folder = [
                record
                for record in self.repository.find_by_cover_key(folder_key)
                if record.runtime.availability_status == "offline"
            ]
            if len(by_folder) == 1:
                return by_folder[0], False, None
            if len(by_folder) > 1:
                return None, False, MatchAmbiguity(candidate, [r.metadata.uuid for r in by_folder])
