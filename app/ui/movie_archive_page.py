...
class MovieArchivePage(QWidget):
    ...
    episode_play_requested = Signal(str, str)
    episode_relink_requested = Signal(str, str)
    episode_folder_requested = Signal(str, str)
    ...
    def _build_ui(self) -> None:
        ...
        self.episode_card = QFrame()
        self.episode_card.setObjectName("movieArchiveCard")
        episode_layout = QVBoxLayout(self.episode_card)
        episode_layout.setContentsMargins(16, 14, 16, 16)
        episode_layout.setSpacing(10)
        episode_header = QHBoxLayout()
        self.episode_title_label = QLabel()
        self.episode_title_label.setObjectName("movieArchiveSectionTitle")
        episode_header.addWidget(self.episode_title_label)
        episode_header.addStretch(1)
        self.episode_details_button = QPushButton()
        self.episode_details_button.setObjectName("movieEpisodeInfoButton")
        self.episode_details_button.setText("")
        self.episode_details_button.setToolTip("剧集详情")
        self.episode_details_button.setIcon(flat_icon("info"))
        self.episode_details_button.setFixedSize(32, 32)
        episode_header.addWidget(self.episode_details_button)
        episode_layout.addLayout(episode_header)
        self.episode_buttons_layout = QGridLayout()
        self.episode_buttons_layout.setHorizontalSpacing(8)
        self.episode_buttons_layout.setVerticalSpacing(8)
        episode_layout.addLayout(self.episode_buttons_layout)
        self.episode_card.setVisible(False)
        content.addWidget(self.episode_card)
        ...
        self.episode_details_button.clicked.connect(self._show_episode_details)
    ...
    def set_record(self, record: MovieRecord) -> None:
        ...
        is_multi_episode = len(record.episodes) > 1
        ...
        available = bool(
            not is_multi_episode
            and runtime.availability_status == "available"
        ...
        self.play_button.setVisible(not is_multi_episode)
        self.media_card.setVisible(not is_multi_episode)
        if is_multi_episode:
            playable = len(record.playable_episodes())
            self.availability_label.setText(
                f"本地可播放 {playable} / {len(record.episodes)} 集"
                if playable
                else "仅档案 / 当前无可播放剧集"
            )
        else:
            self.availability_label.setText(
                "本地可播放" if available else "仅档案 / 当前无本地视频"
            )
        self._set_episode_buttons(record)
    ...
    def _set_episode_buttons(self, record: MovieRecord) -> None:
        while self.episode_buttons_layout.count():
            item = self.episode_buttons_layout.takeAt(0)
            widget = item.widget()
            if widget is not None:
                widget.deleteLater()

        actions = build_episode_actions(record.episodes)
        is_multi_episode = len(actions) > 1
        self.episode_card.setVisible(is_multi_episode)
        if not is_multi_episode:
            return
        self.episode_title_label.setText(f"剧集（{len(actions)}）")
        for index, action in enumerate(actions):
            button = QPushButton(action.label)
            button.setObjectName("movieEpisodeButton")
            button.setEnabled(action.enabled)
            button.setToolTip("点击播放" if action.enabled else "当前离线")
            button.clicked.connect(
                lambda _checked=False, episode_uuid=action.episode_uuid: self._request_episode_play(
                    episode_uuid
                )
            )
            self.episode_buttons_layout.addWidget(button, index // 6, index % 6)

    def _request_episode_play(self, episode_uuid: str) -> None:
        if self.record is not None:
            self.episode_play_requested.emit(
                self.record.metadata.uuid,
                episode_uuid,
            )

    def _show_episode_details(self) -> None:
        if self.record is None or len(self.record.episodes) < 2:
            return
        dialog = MovieEpisodeDialog(self.record, self)
        dialog.relink_requested.connect(self.episode_relink_requested.emit)
        dialog.open_folder_requested.connect(self.episode_folder_requested.emit)
        dialog.exec()
