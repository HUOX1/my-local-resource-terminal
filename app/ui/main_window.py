...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        self.setWindowTitle("本地资源终端 · v0.4.3.1.1")
    ...
    def _build_ui(self) -> None:
        ...
        self.movie_archive_page.episode_play_requested.connect(self._play_movie_episode)
        self.movie_archive_page.episode_relink_requested.connect(self._relink_movie_episode)
        self.movie_archive_page.episode_folder_requested.connect(self._open_movie_episode_folder)
    ...
    def _play_movie_episode(self, movie_uuid: str, episode_uuid: str) -> None:
        if self.catalog is None:
            return
        try:
            episode = self.catalog.episode_for_playback(movie_uuid, episode_uuid)
            handle = self.player_service.play(
                Path(episode.runtime.video_path),
                self.settings,
            )
            self.viewing_service.start_playback(movie_uuid, handle)
            self.refresh_catalog()
            if self.movie_archive_page.movie_uuid == movie_uuid:
                updated = self.catalog.get(movie_uuid)
                if updated is not None:
                    self.movie_archive_page.set_record(updated)
                    self.content_page_stack.setCurrentWidget(self.movie_archive_page)
        except (KeyError, ValueError) as exc:
            QMessageBox.information(self, "当前无法播放", str(exc))
    ...
    def _relink_movie_episode(self, movie_uuid: str, episode_uuid: str) -> None:
        if self.catalog is None:
            return
        path, _ = QFileDialog.getOpenFileName(
            self,
        ...
        if not path:
            return
        try:
            self.catalog.relink_episode(movie_uuid, episode_uuid, Path(path))
        except Exception as exc:
            QMessageBox.warning(self, "关联失败", str(exc))
            return
        self._refresh_movie_archive(movie_uuid)
        self._show_status_message("剧集文件已重新关联", 2500)

    def _open_movie_episode_folder(self, movie_uuid: str, episode_uuid: str) -> None:
        record = self.catalog.get(movie_uuid) if self.catalog else None
        episode = record.episode(episode_uuid) if record is not None else None
        if episode is not None and episode.runtime.video_path:
            QDesktopServices.openUrl(
                QUrl.fromLocalFile(str(Path(episode.runtime.video_path).parent))
            )
    ...
    def _show_movie_context_menu(self, view, pos, model) -> None:
        ...
        if len(records) > 1:
            ...
            if len(record.episodes) > 1:
                ...
                for episode_action in build_episode_actions(record.episodes):
                    play_action = play_menu.addAction(episode_action.label)
                    play_action.setEnabled(episode_action.enabled)
                    play_action.triggered.connect(
                        lambda _checked=False, movie_uuid=record.metadata.uuid, episode_uuid=episode_action.episode_uuid: self._play_movie_episode(
                            movie_uuid,
                            episode_uuid,
                        )
                    )
            ...
            folder_action.setEnabled(
                bool(
                    record.runtime.video_path
                    or any(episode.runtime.video_path for episode in record.episodes)
                )
            )
    ...
    def _play_record(self, record: MovieRecord | None) -> None:
        if record is None:
            return
        if len(record.episodes) > 1:
            QMessageBox.information(self, "选择剧集", "请先选择要播放的剧集。")
            return
        episode = record.single_episode()
        if episode is None:
            QMessageBox.information(self, "当前无法播放", "这部影片目前只有档案，没有已关联的本地视频文件。")
            return
        self._play_movie_episode(record.metadata.uuid, episode.metadata.uuid)

    def _open_folder(self, record: MovieRecord) -> None:
        video_path = record.runtime.video_path or next(
            (
