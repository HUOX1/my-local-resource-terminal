...
logger = logging.getLogger("g3.media_assets")
...
class MediaAssetService:
    ...
    def resolve_preview(self, item_id: str) -> PreviewManifest:
        ...
        manual_or_indexed = self.repository.list_media_assets(item_id)

        by_kind: dict[str, list[tuple[int, int, Path]]] = {}
        source_rank = {"manual": 0, "auto": 1, "generated": 2}
        for asset in manual_or_indexed:
        ...
        def best(kind: str) -> Path | None:
            ...
            if not options:
                return None
            _, _, path = min(options, key=lambda item: (item[0], item[1], str(item[2])))
            return path
