...
| 顶级模块导航 | 已实现，待实机验收 | v0.6.1.9.1：导航柄与抽屉独立锚定窗口右下角；明确可见的 ≡ 按钮支持 hover/click 展开 |
| Games 四槽非对称轨道 | 已实现，待实机验收 | v0.6.1.9.1：按 G1 实机帧重标定为约 20% / 45.5% / 71% / 89%；近邻盒显著放大，形成连续队列；保留串行滚动与真实屏外进出 |
| 真实 3D GLB 游戏盒管线 | 已可用 | 标准高盒 `standard_tall.glb` 已接入；中文节点接口 `盒体` / `封面正面`；旧模型 / 占位模型仅作回退 |
| 磨砂亚克力式盒体材质 | 已实现，待实机验收 | v0.6.1.7：撤销整个盒体 Alpha 透明，改为不透明深度写入 + 冷灰蓝 + clearcoat，避免缺面/排序问题；真正透明留给未来独立外套网格 |
| 产品灯光 / 聚焦实体盒查看 | 已实现，待实机验收 | 聚焦主盒约 `3.42` 且横向锚点约 26%；背景盒放大到约 `1.04–1.28`；悬停约 `±12° / ±4°`；拖动检视 `±55° / ±14°` |
...
| 游戏退出后回到 G3 | 已实现，待实机验收 | 游戏会话开始时最小化并暂停渲染；v0.6.1.9 起退出后恢复进入游戏前的 Windowed / Maximized / Fullscreen 状态，不再强制最大化 |
| 编辑资料 | 已实现，待实机验收 | 标题 / 平台 / 开发商 / 发行商 / 发行年份 / 标签 / 简介 / 收藏备注 |
| System 页面 | 已实现，待实机验收 | 添加游戏、窗口 / 最大化切换、退出 G3、默认启动板块；G3 使用主题自绘无边框窗口栏，保留可缩放 / 最大化 / 最小化 / 关闭 |
| 性能保守化设置 | 已实现，待实机验收 | 默认 `60 FPS`、轻量 `MSAA 3D`；v0.6.1.8 Classic Cyan 进一步压暗背景层级 |
| 媒体素材管理 | 仅入口 | UI 待补齐 |
| 移除收藏 | 仅入口 | 安全确认流程待补齐 |
...
- 关键脚本：
  - `scripts/main.gd`：主界面外壳、后端通讯、预览/启动/状态恢复
  - `scripts/navigation_drawer.gd`：右下角隐藏式顶级导航抽屉
...
1. Windows 实机验收 v0.6.1.9.1：重点看四槽是否真正铺满窗口、副盒明暗/朝向、连续滚轮是否还出现第五盒幽灵、右下导航柄、窗口缩放/最大化/游戏返回。
2. 根据实机结果只微调 G1 四槽参数，不再回到对称五盒布局。
